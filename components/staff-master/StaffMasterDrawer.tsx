...
// components/staff-master/StaffMasterDrawer.tsx
'use client'
...
export default function StaffMasterDrawer({
  ...
  useEffect(() => {
    ...
    if (editing) {
      setForm({
        ...
        staffType: (editing.staffType as '' | 'DRIVER' | 'CLEANER') || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Failed to save staff type'
      setError(message)
    } finally {
      setSaving(false)
    }
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-text-secondary mt-1">
                {editing
                  ? 'Update this role and its system permissions.'
                  : 'Create a staff role and define its system permissions.'}
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-10 py-10 space-y-12">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, staffType: e.target.value as typeof form.staffType })
                }
                className="w-full border-b border-border focus:border-primary focus:ring-0 px-0 py-3 text-lg bg-transparent transition-colors"
              >
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <p className="text-text-secondary text-sm">
                  Enable or disable this role within the system.
                </p>
              </div>
          ...
          <div className="space-y-6">
            <div className="border-b border-border pb-4">
              ...
              <p className="text-text-secondary mt-1">
                Choose what this staff role can access by default.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              {PERMISSION_GROUPS.map((group) => {
                ...
                const allChecked = groupKeys.every((k) =>
                  form.defaultPermissions.includes(k)
                )

                return (
                  <div key={group.category} className="border border-border p-6">
                    <div className="flex items-center justify-between mb-6">
                      <h4 className="font-semibold text-text-primary uppercase tracking-widest text-sm">
                        {group.category}
                      </h4>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          checked={allChecked}
                          onChange={(e) =>
                            toggleCategory(groupKeys, e.target.checked)
                          }
