...
export async function GET(
  ...
  try {
    ...
    if (user.role === 'CUSTOMER') {
      return NextResponse.json(
        { success: false, message: 'Permissions only apply to staff and admins' },
        { status: 400 }
      )
    }
...
export async function PUT(
  ...
  try {
    ...
    if (requestingUserId === params.id) {
      return NextResponse.json(
        { success: false, message: 'You cannot edit your own permissions' },
        { status: 400 }
      )
    }
    ...
    const invalidKeys = permissions.filter(
      (p: string) => !validKeys.includes(p as never)
    )
    ...
    if (targetUser.role === 'CUSTOMER') {
      return NextResponse.json(
        { success: false, message: 'Cannot assign permissions to a customer' },
        { status: 400 }
      )
    }
