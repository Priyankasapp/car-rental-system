...
export default function AddCarPage() {
  ...
  useEffect(() => {
    async function loadMasterData() {
      try {
        ...
        const extractData = async (res: Response): Promise<MasterOption[]> => {
          if (!res.ok) return [];
  ...
  //  Stable reference — no inline onChange in JSX
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded border flex items-center justify-center ${
                          isChecked ? "bg-white text-black" : "bg-white"
                        }`}
                      >
          ...
          <FormSection
            icon={ImageIcon}
            title="Image Gallery"
            description="Upload vehicle images directly to Cloudinary"
          >
            {/* Uses stable handleImageChange — no inline setState */}
            <ImageUploader
              value={imageUrls}
